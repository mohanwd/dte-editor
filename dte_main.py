...
from event_updater import *
from dte_encryption import *
...
class Main(QtWidgets.QMainWindow):
    ...
    def open(self):
        ...
        self.filename = QtWidgets.QFileDialog.getOpenFileName(self, 'Open File',".","(*.dte)")[0]

        if self.filename:
            with open(self.filename,"rb") as file:
                fd = dte_decrypt(file.read())
                soup = BeautifulSoup(fd,"lxml")
                if soup.head:
                    if soup.head.fileid:
                        self.fileid = soup.head.fileid.text
                else:
                    self.fileid = generateFileId()
                self.text.setText(fd)
                eventRegister(self.fileid,'Opened')
    ...
    def save(self):
        ...
        if self.filename:
            ...
            if not self.filename.endswith(".dte"):
              self.filename += ".dte"
            ...
            event_state = ''
            if not self.fileid:
                event_state = 'Modified'
                self.fileid = generateFileId()
            else:
                event_state = 'Created'
                print('file id available')

            with open(self.filename,"wb") as file:
                file_content=self.text.toHtml().replace("<head>","<head><fileid>"+self.fileid+"</fileid>")
                file.write(dte_encrypt(file_content))
            self.changesSaved = True
            eventRegister(self.fileid,event_state)
...
def main():
    ...
    sync_events = threading.Thread(target=db_update)
    main = Main()
    if is_connected("www.google.com"):
        #sync_events.daemon=False
        #sync_events.start()
        print("Syncing Events thread has start")
        main.show()
    else:
        print("Network issue!! Please connect to internet and try")
        main.show()
    sys.exit(app.exec_())
    print("System Closed")
    sync_events.kill()
    print("Syncing closed")
